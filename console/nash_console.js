...
class Nash_console_game {
    ...
    print_optimal() {
        for (let i = 0; i < this.nash_game.x; i++) {
            let line = '';
            for (let j = 0; j < this.nash_game.y; j++) {
                line += this.optimal_choice[i][j][0] + ',' + this.optimal_choice[i][j][1] + ' ';
            }
            console.log(line);
        }
        console.log('-'.repeat(20));
    ...
    play() {
        ...
        for (let col = 0; col < this.nash_game.y; col++) {
            max_val = this.nash_game.table[0][col][0];
            max_ids = [0];
            for (let row = 1; row < this.nash_game.x; row++) {
                if (this.nash_game.table[row][col][0] > max_val) {
                    max_val = this.nash_game.table[row][col][0];
                    max_ids = [row];
                } else if (this.nash_game.table[row][col][0] === max_val) {
                    max_ids.push(row);
                }
            }
            for (let row of max_ids) {
                this.optimal_choice[row][col][0] = 1;
            }
            this.print_optimal();
        }
        // do the same for the 2nd player
        for (let row = 0; row < this.nash_game.x; row++) {
            max_val = this.nash_game.table[row][0][1];
            max_ids = [0];
            for (let col = 1; col < this.nash_game.y; col++) {
                if (this.nash_game.table[row][col][1] > max_val) {
                    max_val = this.nash_game.table[row][col][1];
                    max_ids = [col];
                } else if (this.nash_game.table[row][col][1] === max_val) {
                    max_ids.push(col);
                }
            }
            for (let col of max_ids) {
                this.optimal_choice[row][col][1] = 1;
            }
            this.print_optimal();
        }
        ...
        for (let i = 0; i < this.nash_game.x; i++) {
            for (let j = 0; j < this.nash_game.y; j++) {
                if (this.optimal_choice[i][j][0] === 1 && this.optimal_choice[i][j][1] === 1) {
